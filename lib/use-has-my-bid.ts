...
// 최근 제안순으로 내려오므로, 지금 보고 있는(대개 진행 중인) 매물은 앞쪽에 있다.
const LOOKUP_SIZE = 100;

/**
 * 지금 보고 있는 매물에 **내가 가격을 제안한 적이 있는지**.
 *
 * <p>상세 화면의 기본 탭을 정하는 데 쓴다 — 제안한 사람에게는 상품 설명보다 지금 판이 어떻게
 * 돌아가는지가 먼저다.
 *
 * <p>제안 내역 API는 닉네임을 마스킹해서 내려주므로 그것으로는 내 제안을 가려낼 수 없다.
 * 그래서 마이페이지가 쓰는 "내 제안 목록"을 재사용한다. **정확한 방법은 상세 응답에 플래그를
