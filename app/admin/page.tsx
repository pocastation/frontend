...
// "최근 등록 판매글"은 상태를 가리지 않고 최신순으로 가져오므로 검수 대기·거절 건도 섞인다.
// 공개 상세(/auctions/{id})는 비공개 상태를 404로 막으니, 공개된 것만 상세로 보내고
// 나머지는 판매글 관리 화면으로 보낸다(판매글 관리 목록이 이미 쓰는 것과 같은 기준).
  ...
  return (
    <div>
      ...
      <div className="admin-dashboard-stats mt-6 flex flex-wrap gap-y-4 border-y border-border py-4">
        ...
        <Stat label="판매 중인 상품" value={loading ? "—" : `${data?.liveAuctions ?? 0}건`} />
        <Stat
          label="검수 대기 판매글"
