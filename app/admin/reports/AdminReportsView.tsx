...
function buildParams(status: ReportStatus | "ALL", targetType: ReportTargetType, page: number) {
  const params = new URLSearchParams({ size: String(PAGE_SIZE), page: String(page), targetType });
  if (status !== "ALL") params.set("status", status);
  return params;
}

// 대상마다 부르는 이름이 다르다. 「판매자」와 「작성자」를 한 단어로 뭉개면 관리자가 보는 것이
// 사람인지 가게인지 흐려진다.
const TARGET_COPY: Record<ReportTargetType, { noun: string; owner: string; subtitle: string; href: (id: number) => string }> = {
  AUCTION: { noun: "판매글", owner: "판매자", subtitle: "스타", href: (id) => `/auctions/${id}` },
  EXCHANGE_POST: { noun: "교환글", owner: "작성자", subtitle: "행사", href: (id) => `/exchanges/${id}` },
};
 ...
 * 신고 관리 — 대상 종류 하나를 맡는다.
 *
 * <p>판매글과 교환글은 같은 테이블·같은 처리 흐름이라 화면을 복사하지 않고 {@code targetType}만
 * 다르게 받는다. 라우트를 둘로 나눈 이유는 탭이 진짜 링크여야 하기 때문이다 — 쿼리 파라미터로
 * 상태만 바꾸면 뒤로가기와 새로고침이 탭을 기억하지 못한다.
 *
 * <p>리뷰 신고는 여기 들어오지 않는다. 테이블과 상태 기계가 따로라(RECEIVED/HANDLED) 같은
 * 컴포넌트에 넣으면 분기가 화면 전체에 번진다.
 */
export default function AdminReportsView({ targetType }: { targetType: ReportTargetType }) {
  ...
  // 「내리기」의 실제 값은 서버가 상세에서 알려준다(removalAction). 상세를 열기 전 기본값은
  // 무해한 쪽(반려)으로 둔다 — 대상 종류로 추측하면 종류가 늘 때마다 여기가 먼저 틀린다.
  const [action, setAction] = useState<ResolutionAction>("NONE");
  ...
  const fetchList = useCallback(
    async (status: ReportStatus | "ALL") => {
      setLoading(true);
      try {
        const res = await fetchWithAuth<AdminReportListResponse>(`/api/admin/reports?${buildParams(status, targetType, 0)}`);
  ...
  async function loadMore() {
    ...
    setLoadingMore(true);
    try {
      const res = await fetchWithAuth<AdminReportListResponse>(`/api/admin/reports?${buildParams(statusFilter, targetType, nextPage)}`);
  ...
  const openDetail = useCallback(
    async (targetId: number) => {
      ...
      try {
        const res = await fetchWithAuth<AdminReportDetailResponse>(
          `/api/admin/reports/${targetId}?targetType=${targetType}`,
        );
        setDetail(res);
        setAction(res.removalAction);
      } catch (err) {
        setDetail(null);
  ...
  async function confirmResolve() {
    ...
    setNotice(null);
    try {
      await fetchWithAuth<void>(`/api/admin/reports/${detail.targetId}?targetType=${targetType}`, {
        method: "PATCH",
        body: { action, note: note.trim() },
      });
      setNotice({ kind: "success", text: "신고를 처리했습니다." });
      await Promise.all([openDetail(detail.targetId), fetchList(statusFilter)]);
    } catch (err) {
              ...
              <tbody>
                {reports.length === 0 && !loading ? (
                  ...
                  reports.map((r) => (
                    ...
                    >
                      {/*
                        대상 칸은 제목·부제 두 줄로 통일한다. 판매글은 제목·스타, 교환글은 만날 곳·
                        행사명이 그 자리에 들어간다 — 종류마다 칸을 나누면 종류가 늘 때마다 여기를 고쳐야 한다.

                        교환글에는 썸네일이 없다. 신고 판단이 사진보다 문구·정황에 달려 있고,
                      ...
                      <td className="px-4 py-3"><button type="button" aria-label={`${r.targetTitle ?? copy.noun} 신고 상세`} onClick={(event) => { event.stopPropagation(); void openDetail(r.targetId); }} className={`text-left ${FOCUS_RING}`}>
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="min-w-0">
                            {r.targetSubtitle && <span className="block truncate text-[11px] font-bold text-primary">{r.targetSubtitle}</span>}
                            <span className="block max-w-[180px] truncate font-bold text-text-1">{r.targetTitle ?? "-"}</span>
                          </span>
                        </div>
                      </button></td>
            ...
            <div className="rounded-r3 border border-border bg-surface p-4">
              ...
              <dl className="mt-3 flex flex-col gap-1.5 border-t border-border pt-3 text-[12.5px]">
                ...
                {detail.targetSubtitle && (
                  <div className="flex justify-between gap-2">
                    <dt className="text-text-3">{copy.subtitle}</dt>
                    <dd className="font-semibold text-text-1">{detail.targetSubtitle}</dd>
                  </div>
                )}
                <div className="flex justify-between gap-2">
                  <dt className="text-text-3">{copy.noun}</dt>
                  <dd className="font-semibold">
                    <Link href={copy.href(detail.targetId)} className={`text-primary hover:underline ${FOCUS_RING}`}>
                      바로가기 →
                    </Link>
                  </dd>
                </div>
                ...
                <div className="mt-4 border-t border-border pt-4">
                  ...
                  {targetType === "EXCHANGE_POST" && (
                    <p className="mt-2 text-[11px] leading-relaxed text-text-3">
                      내리면 대기 중인 신청도 함께 마감되고, 작성자와 신청자 모두에게 알림이 갑니다.
                    </p>
                  )}
                </div>
              ) : (
