...
/**
 * 제안 인원수 문구 — 거래 개편 §2.9 확정값. **데스크탑·모바일이 이 함수 하나를 공유한다.**
 *
 * 트리를 둘로 두는 지면(상세)에서 문구를 각자 선언하면 한쪽만 고쳐져 두 화면이 다른 말을 한다.
 * 「모두 읽음」 버튼을 데스크탑 헤더와 모바일 앱바가 같은 엘리먼트로 공유하는 것과 같은 이유다.
 *
 * 🔴 **0건일 때 숫자를 쓰지 않는 것이 이 함수의 핵심이다.** 인원수 공개의 목적은 참여 신호인데
 * 초기 매물은 전부 0명이고, 「0명이 제안했어요」는 **「아무도 안 원하는 물건」**으로 읽혀 목적과
 * 정반대로 작동한다. 그래서 0건은 다음 행동을 권하는 문장으로 바꾼다.
 *
 * ⚠️ 이 수치는 제안 **건수**가 아니라 취소를 뺀 **distinct 제안자 수**다(`offerCount`).
 * 한 사람이 금액을 여러 번 바꿔도 1이다.
 */
export function offerCountLabel(offerCount: number): string {
  return offerCount > 0
    ? `지금 ${offerCount}명이 가격을 제안했어요`
    : "아직 제안이 없어요 · 첫 제안자가 되어보세요";
}
