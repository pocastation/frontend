...
/**
 * 제안이 하나도 없을 때의 문구 — 거래 개편 §2.9 D1 확정값.
 *
 * 🔴 **숫자를 쓰지 않는 것이 이 상수의 핵심이다.** 인원수 공개의 목적은 참여 신호인데 초기
 * 매물은 전부 0명이고, 「0명이 제안했어요」는 **「아무도 안 원하는 물건」**으로 읽혀 목적과
 * 정반대로 작동한다. 그래서 0건은 숫자 대신 다음 행동을 권하는 문장으로 바꾼다.
 *
 * <p>1건 이상일 때의 표시는 {@code components/OfferCounts.tsx}가 맡는다(아이콘 + 「N명」).
 * 문구·표시를 한 곳에 모으는 이유는 상세가 데스크탑·모바일 **트리 두 벌**이라, 각자 선언하면
 * 한쪽만 고쳐져 두 화면이 다른 말을 하기 때문이다.
 */
export const OFFER_EMPTY_HINT = "아직 제안이 없어요 · 첫 제안자가 되어보세요";
