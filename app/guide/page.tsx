...
const GENERAL_STEPS: GuideStep[] = [
  ...
  },
  {
    ...
    body: (
      <>
        {/* 🔴 「제안 추월... 종류별로 끄고 켤 수 있고, 관심 있는 매물만 따로 지정할 수도
            있어요」였다. 추월 알림과 그 설정을 §2.3으로 폐기하면서 **화면이 지키지 못하는
            안내**가 됐다(T40 패턴). 지금 실제로 오는 알림만 적는다. */}
        거래 성사, 결제, 발송 같은 소식은 알림함으로 와요. 거래에 꼭 필요한 알림이라 항상 받아요.
      </>
    ),
  },
];
...
const SELLER_STEPS: GuideStep[] = [
  {
    ...
    body: (
      <>
        제안판매와 즉시판매 중 방식을 고르고, 실물 사진 · 상품 정보 · 가격을 순서대로 입력해요. 정확한
        정보일수록 문의와 분쟁이 줄어요.{" "}
        <Link
          href="/guide/sell"
  ...
  {
    title: "가격 제안 받고 거래 성사",
    body: (
      <>
        제안판매는 정한 기간 동안 구매자들의 가격 제안을 받고, 마감 시각에 가장 높은 금액을 제안한
        구매자와 거래가 성사돼요. 즉시판매는 구매자가 정한 가격으로 바로 사 가요. 거래가 성사되면
        구매자 카드에서 자동으로 결제가 진행돼요.
      </>
    ),
  },
  {
...
const BUYER_STEPS: GuideStep[] = [
  ...
  },
  {
    ...
    body: (
      <>
        제안판매는 1,000원 단위로 가격을 제안하고, 마감 3분 안에 제안이 들어오면 3분씩 연장돼요(최대 3회).
        즉시판매는 정해진 가격으로 바로 구매할 수 있어요. 결제는 미리 등록한 카드로{" "}
        <b className="font-bold text-text-1">거래 성사 즉시 자동</b> 진행되고, 대금은 구매 확정 전까지
        안전하게 보관돼요.
      </>
    ),
  },
