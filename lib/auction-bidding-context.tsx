...
/**
 * 경매 상세의 입찰 상태 — **한 화면에 하나만 띄운다.**
 *
 * <p>모바일과 데스크탑은 지면이 달라 컴포넌트를 따로 두지만, 입찰은 그렇게 나눌 수 없다.
 ...
 *
 * <p>여기 담긴 것: 현재가·입찰수·마감시각(SSE 반영) · 입찰 이력 · 입찰가 스테퍼 · 입찰 요청 ·
 * 배송지 관문 · 최고 입찰자 여부.
 */
...
export function AuctionBiddingProvider({
  ...
  // 다른 사람 입찰로 현재가가 오르면 입력값 하한도 따라 올려야 한다. 단 사용자가 직접 고른 값은 존중한다.
  ...
  const onAddressSaved = useCallback(
    (afterSave?: () => void) => {
      ...
      markRegistered();
      toast.show({ variant: "success", text: "배송지를 등록했어요.", sub: "이제 입찰할 수 있어요." });
      afterSave?.();
    },
