...
const AFTER = [
  ...
  {
    k: "발송하고 운송장을 넣어요",
    v: "3영업일까지 발송하지 않으면 주문이 자동 취소돼요. 주말 · 공휴일은 세지 않아요.",
  },
  {
    // 실지급 배관이 아직 없다(#281). "입금돼요"는 사실이 아니라 약속이었다 — /guide 와 같은 문장을 쓴다.
    k: "구매 확정되면 정산돼요",
    v: "수수료를 뺀 정산 금액이 확정되고, 등록해 두신 정산계좌로 지급돼요. 별도로 신청할 것은 없어요. 지급 일정은 결제 수단과 정산 주기가 확정되는 대로 공지로 안내해 드려요.",
  },
];
              ...
              <div>
                ...
                <div aria-hidden="true" className="mt-2 flex gap-1.5">
                  {PERIODS.map((p, i) => (
                    <span
                      key={p}
                      className={`flex h-10 flex-1 items-center justify-center rounded-[4px] border text-[13px] font-bold tabular-nums ${
                        i === 1 ? "border-primary text-primary" : "border-border-2 text-text-2"
                      }`}
                    >
                      {p}
                    </span>
