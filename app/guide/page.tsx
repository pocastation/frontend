...
const GENERAL_STEPS: GuideStep[] = [
  {
    // 본인인증 언급을 뺐다(#281) — 대행사 계약 전이라 사용자가 완료할 수단이 아예 없다.
    // 도입되면 이 문단으로 돌아온다. 대신 실제로 존재하는 요건(정산계좌)을 적는다.
    title: "회원가입",
    body: (
      <>
        이메일로 가입하면 <b className="font-bold text-text-1">받은 인증 메일의 링크를 눌러야 가입이
        끝나요.</b> 카카오 · 네이버 · 구글은 링크를 누를 필요 없이 바로 가입돼요. 가입을 마치면 곧바로
        입찰 · 구매할 수 있고, 판매를 시작할 때는 정산계좌를 먼저 등록해요.
      </>
    ),
  },
  {
...
const SELLER_STEPS: GuideStep[] = [
  ...
  },
  {
    ...
    body: (
      <>
        {/* 실지급은 아직 배관이 없다(#281) — "입금돼요"는 사실이 아니라 약속이었다.
            확정까지가 지금 실제로 일어나는 일이고, 지급 시점은 결제·정산 수단이 정해져야 말할 수 있다. */}
        구매자가 구매 확정을 누르면 수수료를 뺀 정산 금액이 확정돼요. 별도로 신청할 것은 없고,
        등록해 두신 정산계좌로 지급돼요. 지급 일정은 결제 수단과 정산 주기가 확정되는 대로 공지로
        안내해 드릴게요.
      </>
    ),
  },
